...
function Swatch({ color }) {
  ...
  return (
    <div style={{
      width:11, height:11, borderRadius:'50%', background:color, flexShrink:0,
      border: white ? `1px solid ${C.hairline}` : 'none',
    }}/>
  )
}
...
export default function ProductCard({ product, wishlisted, onWishlist, delay = 0, vendorView = false }) {
  ...
  return (
    <div
      ...
      style={{ cursor:'pointer', animation:`fadeUp 0.5s ease ${delay}ms both` }}
    >
      {/* Image */}
      <div style={{ background: product.image ? C.cloud : (product.cardColor || C.cloud), aspectRatio:'3/4', overflow:'hidden', position:'relative' }}>
        ...
        {product.badge && (
          <div style={{
            ...
            background: BADGE_BG[product.badge] || C.aloe,
        ...
        {!vendorView && (
          <div style={{
            position:'absolute', bottom:0, left:0, right:0, padding:'0 10px 10px',
            transform: hov ? 'translateY(0)' : 'translateY(110%)',
            transition:'transform 0.25s cubic-bezier(0.25,0.46,0.45,0.94)',
          }}>
            <button style={{
              width:'100%', background:C.ink, color:'#fff',
              fontSize:12, fontWeight:500, padding:'10px', borderRadius:9999,
      ...
      <div style={{ paddingTop:10 }}>
        {product.colors && product.colors.length > 0 && (
          <div style={{ display:'flex', gap:4, marginBottom:7, alignItems:'center' }}>
            ...
            <span style={{ fontSize:11, color:C.stone, marginLeft:2 }}>
              {product.colors.length} {product.colors.length === 1 ? 'color' : 'colors'}
            </span>
          </div>
        )}
        <div style={{ fontSize:13, fontWeight:500, marginBottom:2, lineHeight:1.35 }}>
          {product.name}
        </div>
        <div style={{ fontSize:11, color:C.mute, marginBottom:3 }}>
          {product.label}
        </div>
        {vendor && !vendorView && (
          <div style={{ fontSize:11, color:C.stone, marginBottom:5 }}>
            by {vendor.name}
          </div>
        )}
        {vendorView && (
          <div style={{ fontSize:11, color:C.mute, marginBottom:5, textTransform:'capitalize' }}>
            {product.cat} · {product.sub}
          </div>
        )}
        <div style={{ display:'flex', gap:6, alignItems:'baseline' }}>
          <span style={{ fontSize:14, fontWeight:600, color: sale ? C.sale : C.ink }}>
            ${product.price.toFixed(2)}
          </span>
          {sale && <>
            <span style={{ fontSize:12, color:C.stone, textDecoration:'line-through' }}>
              ${product.orig.toFixed(2)}
            </span>
            <span style={{ fontSize:12, fontWeight:500, color:C.sale }}>
              {pct}% off
            </span>
          </>}
        </div>
