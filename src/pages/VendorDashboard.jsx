...
function StarRating({ rating }) {
  return (
    <div style={{ display:'flex', gap:1 }}>
      {[1,2,3,4,5].map(i => <Star key={i} f={i <= Math.round(rating)} />)}
    </div>
  )
}
    ...
    >
      <div
        ...
        style={{
          ...
          padding:36, width:520, maxWidth:'90vw',
      ...
      >
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
          {[
            { label:'Product Name',    placeholder:'e.g. Classic White Sneaker', type:'text'   },
            { label:'Price ($)',       placeholder:'0.00',                        type:'number' },
            { label:'Original Price ($) — leave blank if no discount', placeholder:'0.00', type:'number' },
            ...
            <div key={label}>
              ...
              <input
                ...
                style={{
                  ...
                  border:`1px solid ${C.hairline}`, fontSize:14, outline:'none',
          ...
          <div>
            ...
            <select style={{
              width:'100%', padding:'10px 14px', borderRadius:8,
              border:`1px solid ${C.hairline}`, fontSize:14, outline:'none',
              background:C.canvas, cursor:'pointer',
            }}>
              <option value="">Select a category…</option>
              {Object.values(CAT_META).map(cat => (
                <optgroup key={cat.id} label={cat.label}>
                  {cat.subcategories.filter(s => s.id !== 'all').map(sub => (
                    <option key={sub.id} value={`${cat.id}/${sub.id}`}>{sub.label}</option>
                  ))}
                </optgroup>
              ))}
            </select>
          ...
          <div>
            ...
            <div
              style={{
                border:`2px dashed ${C.hairline}`, borderRadius:8,
                padding:'32px 24px', textAlign:'center', cursor:'pointer',
                color:C.stone, fontSize:13, transition:'all 0.15s',
              }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = C.ink; e.currentTarget.style.color = C.ink }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = C.hairline; e.currentTarget.style.color = C.stone }}
            >
        ...
        <div style={{ display:'flex', gap:10, marginTop:28 }}>
          <button
            onClick={onClose}
            style={{ flex:1, padding:'12px', borderRadius:9999, background:C.cloud, border:'none', cursor:'pointer', fontSize:14, fontWeight:500 }}
          >
            Cancel
          </button>
          <button style={{ flex:2, padding:'12px', borderRadius:9999, background:C.ink, color:'#fff', border:'none', cursor:'pointer', fontSize:14, fontWeight:500 }}>
            Publish Product
          </button>
        </div>
      </div>
    ...
    <div style={{ background:C.cloud, minHeight:'80vh' }}>
      ...
      <div style={{ background:C.canvas, borderBottom:`1px solid ${C.hairline}`, padding:'28px 40px' }}>
        <div style={{ maxWidth:1440, margin:'0 auto', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div>
            <div style={{ fontSize:11, fontWeight:600, letterSpacing:'0.1em', color:C.mute, textTransform:'uppercase', marginBottom:6 }}>
              Vendor Dashboard
            </div>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:8, marginTop:6 }}>
              ...
              <span style={{ fontSize:13, color:C.mute }}>
                {vendor.rating} · {vendor.totalSales.toLocaleString()} total sales
              </span>
            </div>
          </div>
          <button
      ...
      <div style={{ maxWidth:1440, margin:'0 auto', padding:'32px 40px' }}>
        ...
        <div style={{ background:C.canvas, border:`1px solid ${C.hairline}`, borderRadius:10, marginBottom:16 }}>
          <div style={{
            padding:'18px 24px', borderBottom:`1px solid ${C.hairline}`,
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:'24px 16px', padding:24 }}>
            {myProducts.map((p, i) => (
              <ProductCard key={p.id} product={p} vendorView delay={i * 50} />
            ))}
          </div>
        </div>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            ...
            <tbody>
              {RECENT_ORDERS.map(row => (
                <tr
                  key={row.id}
                  style={{ borderBottom:`1px solid ${C.hairline}`, transition:'background 0.1s' }}
                ...
                >
                  ...
                  <td style={{ padding:'12px 20px' }}>
                    <span style={{
                      ...
                      background: row.sc + '22', color: row.sc, letterSpacing:'0.04em',
                    }}>{row.status}</span>
                  </td>
                </tr>
              ))}
